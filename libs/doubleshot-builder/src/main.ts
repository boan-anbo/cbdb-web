...
function runMainProcess(mainFile: string, electron: any, args: DevArgs, env?: Record<string, string>) {
  ...
  return spawn(electron ?? 'node', [mainFile, ...devArgs], {
    stdio: 'inherit',
    env: { ...process.env, ...env }
  ...
  const prebuild = async () => {
    ...
    for (let i = 0; i < tsupConfigs.length; i++) {
      ...
      const onSuccess: TsupOptions['onSuccess'] = async () => {
        ...
        child = runMainProcess(mainFile!, electron, dsArgs, dsEnv as Record<string, string>)
      }
